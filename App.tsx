...
const DEFAULT_COMPANY: CompanyInfo = {
  name: "Papelaria Criativa & Cia",
  logo: "https://images.unsplash.com/photo-1513542789411-b6a5d4f31634?w=200&h=200&fit=crop",
...
const App: React.FC = () => {
  ...
  const handleDownload = async () => {
    ...
    // Switch to capture mode for a clean look
    setIsCapturing(true);

    // Give React time to re-render the clean state
    setTimeout(async () => {
      try {
        const canvas = await html2canvas(budgetRef.current!, {
          ...
          onclone: (clonedDoc) => {
            // Optional: adjust elements in the clone if needed
  ...
  return (
    <div className={`min-h-screen ${isCapturing ? 'bg-white' : 'bg-slate-50'}`}>
      <div ref={budgetRef} data-capture-container className={isCapturing ? "w-[1200px] mx-auto bg-white p-10" : "pb-24"}>
        {/* Header - Always at the top in the captured area */}
        <Header info={DEFAULT_COMPANY} isCapturing={isCapturing} />
        
        <main className={`max-w-6xl mx-auto px-4 ${isCapturing ? 'px-0' : ''}`}>
          {/* Upload Area - Hidden during capture */}
          {!isCapturing && (
            <section className="bg-white p-8 rounded-2xl shadow-sm border border-slate-200 mb-8 no-print">
              <div className="flex flex-col md:flex-row items-center gap-8">
                <div className="flex-1 w-full">
                  <h2 className="text-xl font-bold text-slate-800 mb-2">Novo Orçamento Inteligente</h2>
                  <p className="text-slate-500 mb-6">Tire uma foto da sua lista de produtos para que nossa IA organize tudo para você.</p>
                  ...
                  <div className="relative group">
                    ...
                    />
                    <div className={`border-2 border-dashed rounded-xl p-10 flex flex-col items-center justify-center transition-all ${isAnalyzing ? 'bg-slate-50 border-slate-200' : 'border-emerald-200 bg-emerald-50 group-hover:bg-emerald-100 group-hover:border-emerald-300'}`}>
                      {isAnalyzing ? (
                        ...
                        <>
                          <div className="w-14 h-14 bg-emerald-500 rounded-full flex items-center justify-center text-white mb-4 shadow-lg shadow-emerald-200">
                            <svg className="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z"></path></svg>
                          </div>
                          <p className="text-slate-700 font-semibold">Clique para selecionar ou arraste a foto</p>
                          <p className="text-slate-400 text-sm mt-1">PNG, JPG até 10MB</p>
                        </>
                      )}
                    </div>
                  </div>
                ...
                {previewUrl && (
                  <div className="w-full md:w-64 flex-shrink-0">
                    <p className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-2">Preview da Foto</p>
                    <div className="rounded-xl overflow-hidden border border-slate-200 shadow-sm aspect-[3/4]">
          ...
          {/* Items Table */}
          <section className={`bg-white rounded-2xl ${isCapturing ? 'border-none' : 'shadow-lg border border-slate-200 overflow-hidden'}`}>
            {!isCapturing && (
              <div className="p-6 md:p-8 bg-slate-50 border-b border-slate-200 flex justify-between items-center no-print">
                ...
                <button 
                  ...
                  className="flex items-center gap-2 bg-white px-4 py-2 rounded-lg border border-slate-200 text-slate-700 font-medium hover:bg-slate-50 transition-colors shadow-sm"
                >
            ...
            <div className={`p-10 flex flex-col items-end border-t border-slate-200 ${isCapturing ? 'bg-white' : 'bg-slate-50'}`}>
              <div className="w-full md:w-96 space-y-4">
                ...
                <div className="p-5 rounded-2xl border-2 border-emerald-500 bg-white mt-6 shadow-sm">
                  <p className="text-xs text-emerald-600 font-black uppercase mb-1 tracking-widest text-center">Valor para Pagamento Pix</p>
                  <p className="text-3xl font-black text-emerald-600 text-center">{formatCurrency(summary.total - summary.discountCash)}</p>
                </div>
              </div>
            </div>
            ...
            <div className="p-10 text-center text-slate-400 text-xs border-t border-slate-100">
              ...
              <p>{DEFAULT_COMPANY.name} agradece a sua preferência!</p>
